...
## Version History
- **v1.7.0** (Nov 21, 2025): Added comprehensive timezone support - frontend automatically detects browser timezone and sends offset to backend, scheduler and ETA calculations now use user's local timezone (no more UTC confusion), task execution and auto-resume work correctly across all timezones, feed mode uses UTC for timezone-independent duration timing, configurable via USER_TZ_OFFSET environment variable (e.g., 660 for AEDT/UTC+11), upcoming tasks widget now shows correct ETAs for imminent tasks
- **v1.6.0** (Nov 20, 2025): Fully automated 24/7 operation - wavemakers automatically resume to the correct preset on startup/page refresh by searching backwards through schedule (up to 7 days, respecting day-of-week filters), scheduler execution fixed with 30-second checks using ±30 second time window matching (prevents missing scheduled tasks), comprehensive execution logging shows when tasks trigger/execute/skip with detailed context
- **v1.5.3** (Nov 19, 2025): UI polish updates - compact time display (1h34m format), system time shows abbreviated date/day format (Thursday Nov 20th 5:35am), upcoming tasks with edit icon buttons, version number displayed in footer
...
- **Automation Scheduler Backend**: Fully automated 24/7 preset activation system with 30-second precision execution and ±30 second time window matching (ensures tasks never get missed). Features automatic startup resume that searches backwards up to 7 days to find and activate the most recent scheduled preset (respecting day-of-week filters). Supports scheduled tasks with optional day-of-week filtering, database persistence (ScheduledTaskRow), and automatic execution via JobScheduler running every 30 seconds. REST API provides CRUD operations for scheduled tasks (/api/automation/scheduled) and upcoming task queries with timezone support (?tz_offset parameter). Comprehensive logging tracks trigger, execution, skip, and failure events with detailed context. Integrated with PresetManager for seamless automated preset switching throughout the day - wavemakers always reflect current schedule even after system restart or page refresh. **Timezone Support**: All task scheduling, execution, and ETA calculations use user's local timezone (detected from browser or configured via USER_TZ_OFFSET env var). Tasks are interpreted as local time (e.g., "14:00" means 2pm user time, not UTC). Feed mode duration timing uses UTC for timezone-independent accuracy.
