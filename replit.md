...
## Recent Changes
- **v2.2.6** (2025-11-26): Background scroll flicker fix - Replaced position-switching scroll logic (absolute ↔ fixed) with transform-based approach. Background now stays `position: fixed` always and uses `translateY()` for smooth GPU-accelerated scrolling. Eliminates flicker caused by position changes triggering layout recalculations. Tall images scroll with content and lock at bottom; landscape images stay fixed and cover viewport.
